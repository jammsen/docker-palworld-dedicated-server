...
function startServer() {
    ...
    if [[ ! -z ${COOP_PLAYER_MAX_NUM+x} ]]; then
        ...
        sed -E -i "s/CoopPlayerMaxNum=[+-]?([0-9]*[.])?[0-9]+/CoopPlayerMaxNum=$COOP_PLAYER_MAX_NUM/" ${GAME_PATH}/Pal/Saved/Config/LinuxServer/PalWorldSettings.ini
    fi
    ...
    if [[ ! -z ${REGION+x} ]]; then
        ...
        sed -E -i "s/Region=[a-zA-Z]*/Region=$REGION/" ${GAME_PATH}/Pal/Saved/Config/LinuxServer/PalWorldSettings.ini
    fi
    if [[ ! -z ${USEAUTH+x} ]]; then
