...
### I changed the `BaseCampWorkerMaxNum` setting, why didn't this update the server?
  ...
  > This is a confirmed bug. Changing `BASE_CAMP_WORKER_MAXNUM` in the `PalWorldSettings.ini` has no effect on the server. There are tools out there to help with this, like this one: <https://github.com/legoduded/palworld-worldoptions>
